...
const { hamidmd } = require('./lib/system');
const config = require('./config');
const events = require('./hamid');
...
function hamidLog(message, type = 'info') {
    const icons = { info: '📝', success: '✅', error: '❌', warning: '⚠️', debug: '🐛' };
    console.log(`${icons[type] || '📝'} [HAMID MD] ${new Date().toISOString()}: ${message}`);
}
...
hamidLog(`Loading ${pluginFiles.length} plugins...`, 'info');
for (const file of pluginFiles) {
    try { require(path.join(pluginsDir, file)); }
    catch (e) { hamidLog(`Failed to load plugin ${file}: ${e.message}`, 'error'); }
}
    ...
    socket.ev.on('call', async (calls) => {
        try {
            ...
            for (const call of calls) {
                ...
                hamidLog(`Auto-rejected call for ${number} from ${call.from}`, 'info');
            }
        } catch (err) {
            hamidLog(`Anti-call error for ${number}: ${err.message}`, 'error');
        }
    });
}
...
function setupAutoRestart(socket, number) {
    ...
    socket.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            hamidLog(`Connection closed for ${number}: ${statusCode} - ${errorMessage}`, 'warning');

            if (statusCode === 401 || (errorMessage && errorMessage.includes('401'))) {
                hamidLog(`Manual unlink detected for ${number}, cleaning up...`, 'warning');
            ...
            if (isNormalError) { hamidLog(`Normal closure for ${number}, no restart needed.`, 'info'); return; }

            if (restartAttempts < maxRestartAttempts) {
                restartAttempts++;
                hamidLog(`Reconnecting ${number} (${restartAttempts}/${maxRestartAttempts}) in 10s...`, 'warning');
                const sanitizedNumber = number.replace(/[^0-9]/g, '');
                activeSockets.delete(sanitizedNumber);
                socketCreationTime.delete(sanitizedNumber);
                socket.ev.removeAllListeners();
                ...
                try {
                    ...
                    await hamidPair(number, mockRes);
                } catch (e) { hamidLog(`Reconnection failed for ${number}: ${e.message}`, 'error'); }
            } else {
                hamidLog(`Max restart attempts reached for ${number}.`, 'error');
            }
        }
...
async function hamidPair(number, res = null) {
    ...
    try {
        ...
        connectionLockKey = `hamid_lock_${sanitizedNumber}`;
        ...
        if (!existingSession) {
            hamidLog(`No MongoDB session for ${sanitizedNumber} — new pairing required`, 'info');
            if (fs.existsSync(sessionPath)) {
                await fs.remove(sessionPath);
                hamidLog(`Cleaned leftover local session for ${sanitizedNumber}`, 'info');
            }
        } else {
            ...
            hamidLog(`🔄 Restored existing session from MongoDB for ${sanitizedNumber}`, 'success');
        }
        ...
        if (!conn.authState.creds.registered) {
            ...
            try {
                ...
                hamidLog(`Failed to request pairing code: ${error.message}`, 'error');
                if (res && !res.headersSent) {
            ...
            hamidLog(`✅ Using existing session for ${sanitizedNumber}`, 'success');
            if (res && !res.headersSent) {
        ...
        conn.ev.on('creds.update', async () => {
            ...
            if (isNewSession) {
                hamidLog(`🎉 NEW user ${sanitizedNumber} successfully registered!`, 'success');
            }
        });

        // Anti-delete
        conn.ev.on('messages.update', async (updates) => {
            await handleAntidelete(conn, updates, hamidStore);
        });
        ...
        conn.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                await hamidmd(conn);
                hamidLog(`Connected: ${sanitizedNumber}`, 'success');
            ...
            if (connection === 'close') {
                ...
                if (reason === DisconnectReason.loggedOut) hamidLog(`Session logged out.`, 'error');
            }
        });
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© HAMID MD',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:HAMID MD BOY\nORG:HAMID MD BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© HAMID MD' } }
                    }},
                ...
                if (isCmd) {
                    ...
                    if (cmd) {
                        ...
                        } catch (e) { hamidLog(`PLUGIN ERROR [${command}]: ${e.message}`, 'error'); }
                    }
                }
            ...
            } catch (e) { hamidLog(`Message handler error: ${e.message}`, 'error'); }
        });

    } catch (err) {
        hamidLog(`HAMID MD Pair error: ${err.message}`, 'error');
...
router.get('/code', async (req, res) => { if (!req.query.number) return res.json({ error: 'Number required' }); await hamidPair(req.query.number, res); });
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'HAMID MD is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 HAMID MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
...
async function autoReconnectFromMongoDB() {
    try {
        hamidLog('Attempting auto-reconnect from MongoDB...', 'info');
        const numbers = await getAllNumbersFromMongoDB();
        if (!numbers.length) { hamidLog('No numbers in MongoDB', 'info'); return; }
        for (const number of numbers) {
        ...
        hamidLog('Auto-reconnect completed', 'success');
    } catch (e) { hamidLog(`autoReconnectFromMongoDB error: ${e.message}`, 'error'); }
}
...
process.on('uncaughtException', (err) => {
    hamidLog(`Uncaught exception: ${err.message}`, 'error');
});
